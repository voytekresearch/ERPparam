...
class ERPparam():
    ...
    def _fit_peak_guess(self, guess):
        ...
        # In rare cases the signal amplitudes yield different param bounds than the guess params generated by iterative signal fitting
        # so these have to be manually adjusted
        for g_idx in range(guess.shape[0]): #  loop through guess peaks
            # check whether amplitudes of the guess are within the parameter bounds for signal amplitude, and if not, manually set them
            if guess[g_idx][1] < np.min(self.signal):
