...
class ERPparam():
    ...
    def __init__(self, peak_width_limits=(0.01, 10), max_n_peaks=20, 
                 ...
                 fit_offset=False, gauss_overlap_thresh = 0.75, maxfev = 500, 
                 verbose=True):
        ...
        try:
            ...
            if self.fit_offset:
                ...
                if self.peak_mode == 'skewed_gaussian':
                    self.gaussian_params_ = final_sigmultigauss_params[3:].reshape(-1, 4) # get gauss params alone, and reshape to (n peaks,4)
                else:
    ...
    def _fit_peak_guess(self, guess, signal):
        ...
        try:
            if self.peak_mode == "skewed_gaussian":
                gaussian_params, _ = curve_fit(skewed_gaussian_function, 
                                               self.time, signal, p0=guess, 
                                               maxfev=self.maxfev, 
                                               bounds=gaus_param_bounds)
            elif self.peak_mode == "gaussian":
                gaussian_params, _ = curve_fit(gaussian_function, self.time, 
                                               signal, p0=guess, 
                                               maxfev=self.maxfev, 
                                               bounds=gaus_param_bounds)
